...
def load_cbm(load_dir, device):
    ...
    W_c = torch.load(os.path.join(load_dir ,"W_c.pt"), map_location=device, weights_only=True)
    W_g = torch.load(os.path.join(load_dir, "W_g.pt"), map_location=device, weights_only=True)
    b_g = torch.load(os.path.join(load_dir, "b_g.pt"), map_location=device, weights_only=True)

    proj_mean = torch.load(os.path.join(load_dir, "proj_mean.pt"), map_location=device, weights_only=True)
    proj_std = torch.load(os.path.join(load_dir, "proj_std.pt"), map_location=device, weights_only=True)
