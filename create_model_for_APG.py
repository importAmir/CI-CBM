...
def create_backbone_model(backbone_type: str, all_args: dict):
    """
    Create a backbone model based on the provided configuration.

    Args:
        backbone_type (str): Name of the model backbone.
        all_args (dict): Dictionary containing configuration parameters.

    Returns:
        timm model instance
    """

    if 'SelfPrompt' in backbone_type:
        # Check for pretrained settings
        pretrained = all_args.get('first_pretrain', False)
        pretrained_path = all_args.get('pretrain_path', None)

        # Extract necessary parameters from all_args
        drop_rate = all_args.get('drop', 0.0)
        drop_path_rate = all_args.get('drop_path', 0.0)
        drop_block_rate = None  # Not specified in provided code
        immediate_layer = all_args.get('immediate_layer')  # Needs user input
        ...
        prompt_config = all_args.get('prompt_config', {})
        extra_token_nums = prompt_config.get('more_prompts', 1) + 1

        # Create the model
        # model = create_model(
        #     backbone_type,
        #     pretrained=pretrained,
        #     pretrained_path=pretrained_path,  # If available
        #     num_classes=0,  # Feature extraction mode
        #     drop_rate=drop_rate,
        #     drop_path_rate=drop_path_rate,
        #     drop_block_rate=drop_block_rate,
        #     extra_token_nums=extra_token_nums,
        #     immediate_layer=immediate_layer,
        #     immediate_after_norm=immediate_after_norm,
        ...
        if pretrained_path:
            ...
            state_dict = torch.load(pretrained_path, 'cpu')['backbone_fc']
            ...
            # checkpoint = torch.load(pretrained_path, 'cpu')
            # err = model.load_state_dict(checkpoint["backbone_fc"], strict=False)
            # logger.warning(err)
...
def create_SelfPromptDeit_mytiny(dataset):
    import data_utils
    base_path = data_utils.MODEL_ROOTS["SelfPromptDeit_mytiny"]
    all_args = {
        'first_pretrain': True,
        'drop': 0.0,
        'drop_path': 0.1,
        'immediate_layer': 9,
        'immediate_after_norm': None,
        'replace_cls': False,
        'prompt_config': {'more_prompts': 1}
    }
    if dataset == "imagenetsubset":
        all_args['pretrain_path'] = f"{base_path}/my_deit_B50_85.5_no_cls/net_0_task_0.pth"
    elif dataset == "cifar100":
        all_args['pretrain_path'] = f"{base_path}/my_deit_tiny_80_CIFAR100_B50_nocls/net_0_task_0.pth"
    else:
        assert False, f"{dataset}"
        
    backbone_type = "SelfPromptDeit_mytiny"  # Fill in with the correct model name


    model = create_backbone_model(backbone_type, all_args)
...
# random_input = torch.randn(2, 3, 224, 224).to(device)
