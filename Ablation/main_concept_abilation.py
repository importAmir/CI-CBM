...
parser.add_argument("--concept_percentage", type=float, default=100.0, help="Percentage of concepts to use (0-100)")
...
if __name__=='__main__':
    ...
    for exp_id in range(args.n_experiences):
        ...
        # Sample a percentage of concepts if specified
        if args.concept_percentage < 100.0:
            num_concepts = max(1, int(len(selected_concepts) * args.concept_percentage / 100.0))
            selected_concepts = np.random.choice(selected_concepts, size=num_concepts, replace=False).tolist()
            print(f"Using {len(selected_concepts)} concepts ({args.concept_percentage}% of available concepts)")
        ...
        if args.concept_percentage < 100.0:
            save_name = save_name + f"_concepts_{args.concept_percentage}pct"
