...
def add_noise_to_features(features, snr):
    """
    Add noise to features to achieve desired SNR.
    ...
    # Verify actual SNR
    actual_signal_power = torch.mean(features ** 2)
    actual_noise_power = torch.mean(noise ** 2)
    actual_snr = 10 * torch.log10(actual_signal_power / actual_noise_power)
    print(f"Requested SNR: {snr} dB, Actual SNR: {actual_snr.item():.2f} dB")
...
parser.add_argument("--snr", type=float, default=None, help="Signal-to-Noise Ratio for CLIP features. If None, no noise is added.")
...
if __name__=='__main__':
    ...
    for exp_id in range(args.n_experiences):
        ...
        with torch.no_grad():
            ...
            # Add noise to text features if SNR is specified
            if args.snr is not None:
                print(f"Adding noise to text features with SNR = {args.snr} dB")
                text_features = add_noise_to_features(text_features, args.snr)
                print(f"Adding noise to image features with SNR = {args.snr} dB")
                image_features = add_noise_to_features(image_features, args.snr)
            ...
            # Add noise to CLIP features if SNR is specified
            # if args.snr is not None:
            #     print(f"Adding noise to CLIP features with SNR = {args.snr} dB")
            #     clip_features = add_noise_to_features(clip_features, args.snr)
            ...
            if args.snr is not None:
                print(f"Adding noise to Val image features with SNR = {args.snr} dB")
                val_image_features = add_noise_to_features(val_image_features, args.snr)

            # Compute validation clip features
            val_clip_features = val_image_features @ text_features.T

            # if args.snr is not None:
            #     print(f"Adding noise to Val CLIP features with SNR = {args.snr} dB")
            #     val_clip_features = add_noise_to_features(val_clip_features, args.snr)
        ...
        if args.snr is not None:
            save_name = save_name + f"_snr_{args.snr}"
