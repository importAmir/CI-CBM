...
def train_saga(linear, loader, lr, nepochs, lam, alpha, group=True, verbose=None, 
    ...
    with ch.no_grad(): 
        ...
        for t in tqdm(range(nepochs)): 
            ...
            for batch in loader: 
                ...
                if family == 'multinomial': 
                    ...
                    target = I[y].to(weight.device) # change to OHE (One-Hot Encoding)

                    # Calculate new scalar gradient 
                    logits = F.softmax(out, dim=-1)
                    # logits = F.sigmoid(linear(X))
                elif family == 'gaussian': 
                    if w is None: 
        ...
        del batch, X, y, idx, out, loss, target, logits, total_loss
        del a, a_prev
        del w_grad, w_grad_prev, w_saga, weight, weight_new
        del b_grad, b_grad_prev, b_saga, bias, bias_new
        ch.cuda.empty_cache()
        return {
            "a_table": a_table.cpu(), 
            "w_grad_avg": w_grad_avg.cpu(), 
